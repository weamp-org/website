...
- Tailwind v4 is CSS-first config: there is no `tailwind.config.js`. Theme tokens are declared in `src/app/globals.css` via `@theme inline`; dark mode uses the `prefers-color-scheme` media query, not a `.dark` class.
